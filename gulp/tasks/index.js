...
gulp.task("scripts:vendors:min", ["scripts:vendors"], function() {
  return gulp
    .src(config.path.src + "html/js/vendors.js")
    .pipe(plumber())
    .pipe(sourcemaps.init())
    .pipe(uglify())
    .pipe(
...
gulp.task("scripts:app", function() {
  return gulp
    .src(config.path.src + "html/js/app.js")
    .pipe(plumber())
    .pipe(sourcemaps.init())
    .pipe(
      babel({
...
gulp.task("nunjucks", function() {
  return (
    gulp
      .src(config.path.src + "templates/pages/**/*.+(html|nunjucks)")
      .pipe(plumber())
...
gulp.task("build:copy", ["build:clean"], function() {
  return gulp
    .src(config.buildFilesFoldersCopy)
...
gulp.task("waitForStyles", ["styles"], function() {
  return gulp
    .src(config.path.src + "html/css/*.css")
    .pipe(browserSync.stream());
});

gulp.task(
  "waitForVendors",
  ["scripts:vendors", "scripts:vendors:min"],
  function() {
    browserSync.reload();
  }
);
...
gulp.task("waitForHTML", function() {
  return gulp
    .src(config.path.src + "html/**/*.html")
    .pipe(watch(config.path.src + "html/**/*.+(html)"))
    .pipe(plumber())
    .pipe(browserSync.stream());
});
